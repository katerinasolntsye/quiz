  ...
  <main>
      <h1>Фонетика и правила чтения. Часть 2</h1>
        ...
        <p>
          ...
          <span class="lessons_menu" :class="(JSON.parse($store.getters.getUser.payedcourses)[1].lessons.filter(lesson => lesson.title == l.title)[0].unlocked == true)?'unlocked-lesson':'locked-lesson'" @click="openLesson(l); getTitle(l.title)" v-for="l in lessons" :key="l.title" v-html="l.title" style="white-space: pre;"></span>
