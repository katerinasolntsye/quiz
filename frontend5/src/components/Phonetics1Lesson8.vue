      ...
      <h1>Урок 8. Согласные буквы</h1>
		<div id="start-cards-1" :class="openedtaskclass(1)">
			<p>Карточки 1
				...
				<span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[0].goal }}</b> раз</span>
		...
		<div id="start-cards-2" :class="openedtaskclass(2)">
			<p>Карточки 2
				...
				<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[1].goal }}</b> раз</span>
				<span class="start-quiz-down-tip">&#129171;</span></p>
		</div>
		<div id="close-cards-2" :class="closedtaskclass(2)">
			<p>Карточки 2
				<img :class="showtasklock(2)" style="width: 18px;" src="/images/lock.svg" alt="Задание не выполнено" title="Задание не выполнено" />
				<img :class="showtaskunlock(2)" style="width: 21px;" src="/images/unlock.svg" alt="Задание выполнено" title="Задание выполнено" />
				<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[7].tasks[1].goal }}</b> раз</span>
		...
		<CardsComponent id="cards-2" class="quiz" :cards-data="cards2" :task-name="taskName2" :lesson-name="lessonName" :course-name="courseName" />
  </main>
  </div>
</template>
...
export default {
  ...
  mounted() {  
    document.title = "FullEng – Урок 8. Согласные буквы";
  ...
  methods: {
	showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '8. Согласные буквы')[0]
