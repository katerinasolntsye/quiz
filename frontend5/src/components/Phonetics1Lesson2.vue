<template>
  ...
  <!-- <HeaderComponent2 /> -->
  <main>
      <!-- <img id='zb-1' class='sound-btn' src='../assets/sound-off1.svg' alt='Звук' /> -->
      <h1>Урок 2. Гласные звуки</h1>
		<div id="start-cards-1" :class="openedtaskclass(1)">
          <p>Карточки 1
            ...
            <span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[1].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[1].tasks[0].goal }}</b> раз</span>
        ...
        <div id="start-quiz-1" :class="openedtaskclass(2)">
          <p>Тест 1
            ...
            <span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[1].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[1].tasks[1].goal }}</b> раз</span>
            <span class="start-quiz-down-tip">&#129171;</span></p>
        </div>
...
export default {
  ...
  computed: {
	cards1() {
		return this.$store.state.user.cards.cards1
	}
  },
  mounted() {  
    document.title = "FullEng – Урок 2. Гласные звуки";
  ...
  methods: {
	showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '2. Гласные звуки')[0]
