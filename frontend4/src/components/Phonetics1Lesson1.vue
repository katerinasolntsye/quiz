<template>
  ...
  <!-- <HeaderComponent2 /> -->
  <main>
  <!-- <main onmousedown="return false" onselectstart="return false" oncopy="return false"> -->
      <!-- <img id='zb-1' class='sound-btn' src='../assets/sound-off1.svg' alt='Звук' title='Буква Aa' /> -->
      <h1>Урок 1. Введение</h1>
        <div id="start-cards-1" :class="openedtaskclass(1)">
          <p>Карточки 1
            ...
            <span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[0].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[0].tasks[0].goal }}</b> раз</span>
        ...
        <div id="start-quiz-1" :class="openedtaskclass(2)">
          <p>Тест 1
            ...
            <span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[0].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[0].tasks[1].goal }}</b> раз</span>
            <span class="start-quiz-down-tip">&#129171;</span></p>
        </div>
        ...
        <p>ВСТАВКА!!!</p>
        <div v-html="$store.getters.getUser.lessons"></div>
        <p>ВСТАВКА!!!</p>
  </main>
  </div>
</template>
