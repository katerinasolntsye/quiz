      ...
      <h1>Урок 9. Дополнения</h1>
		<div id="start-cards-1" :class="openedtaskclass(1)">
			<p>Карточки 1
				...
				<span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[8].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[8].tasks[0].goal }}</b> раз</span>
		...
		<div id="start-cards-2" :class="openedtaskclass(2)">
			<p>Карточки 2
				...
				<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[8].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[8].tasks[1].goal }}</b> раз</span>
				<span class="start-quiz-down-tip">&#129171;</span></p>
		</div>
...
export default {
  ...
  mounted() {  
    document.title = "FullEng – Урок 9. Дополнения";
  ...
  methods: {
	showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '9. Дополнения')[0]
