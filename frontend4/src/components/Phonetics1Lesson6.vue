<template>
  ...
  <main>
      ...
      <h1>Урок 6. Слог и ударение</h1>
		<div id="start-cards-1" :class="openedtaskclass(1)">
			<p>Карточки 1 <i style="padding-right: 5px;">(по желанию)</i>
				...
				<span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[5].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[5].tasks[0].goal }}</b> раз</span>
		...
		<div id="start-quiz-1" :class="openedtaskclass(2)">
			<p>Тест 1
				...
				<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[5].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[5].tasks[1].goal }}</b> раз</span>
				<span class="start-quiz-down-tip">&#129171;</span></p>
		</div>
...
export default {
  ...
  mounted() {  
    document.title = "FullEng – Урок 6. Слог и ударение";
  ...
  methods: {
	showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '6. Слог и ударение')[0]
