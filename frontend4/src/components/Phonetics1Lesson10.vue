      ...
      <h1>Урок 10. Практика</h1>
		<div id="start-cards-1" :class="openedtaskclass(1)">
			<p>Карточки 1
				...
				<span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[9].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[9].tasks[0].goal }}</b> раз</span>
		...
		<div id="start-cards-2" :class="openedtaskclass(2)">
			<p>Карточки 2
				...
				<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[9].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[9].tasks[1].goal }}</b> раз</span>
				<span class="start-quiz-down-tip">&#129171;</span></p>
		</div>
...
export default {
  ...
  methods: {
	...
	showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '10. Практика')[0]
