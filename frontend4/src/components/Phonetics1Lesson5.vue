  ...
  <main>
      ...
      <h1>Урок 5. Алфавит</h1>
        <div id="start-cards-1" :class="openedtaskclass(1)">
			<p>Карточки 1
				...
				<span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[0].goal }}</b> раз</span>
      ...
      <div id="start-quiz-1" :class="openedtaskclass(2)">
        <p>Тест 1
			...
			<span class="task-done-counter" :class="showtaskcounter(2)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[1].goal }}</b> раз</span>
            <span class="start-quiz-down-tip">&#129171;</span></p>
      </div>
      <div id="close-quiz-1" :class="closedtaskclass(2)">
        <p>Тест 1
            <img :class="showtasklock(2)" style="width: 18px;" src="/images/lock.svg" alt="Задание не выполнено" title="Задание не выполнено" />
			<img :class="showtaskunlock(2)" style="width: 21px;" src="/images/unlock.svg" alt="Задание выполнено" title="Задание выполнено" />
			<span class="task-done-counter" :class="showtaskcounter(3)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[1].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[0].lessons[4].tasks[1].goal }}</b> раз</span>
            <span class="start-quiz-up-tip">&#129169;</span></p>
      </div>
      <QuizComponent id="quiz-1" class="quiz" :quiz-data="questions1" :task-name="taskName3" :lesson-name="lessonName" :course-name="courseName" />
  </main>
  </div>
</template>
...
export default {
  ...
  methods: {
    showtaskcounter(id) {
      ...
      .lessons.filter(c => c.title == '5. Алфавит')[0]
