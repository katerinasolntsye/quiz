      ...
      <h1>Урок 2. Буквы Bb, Cc, Dd</h1>
        <div id="start-cards-1" :class="openedtaskclass(1)">
          <p>Карточки 1
            ...
            <span class="task-done-counter" :class="showtaskcounter(1)">Выполнено <b>{{ JSON.parse($store.getters.getUser.payedcourses)[1].lessons[1].tasks[0].completations }}</b> из <b>{{ JSON.parse($store.getters.getUser.payedcourses)[1].lessons[1].tasks[0].goal }}</b> раз</span>
...
export default {
  ...
  mounted() {  
    document.title = "FullEng – Урок 2. Буквы Bb, Cc, Dd";
